      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="bg-white px-6 py-4 border-b border-[#E5E5EA] flex items-center justify-between sticky top-0 z-10">
            ...
            <button onClick={onClose} className="p-2 bg-[#F2F2F7] hover:bg-[#E5E5EA] rounded-full transition-colors text-[#8E8E93]">
              <X size={20} />
            </button>
          </div>
          <div className="p-4 space-y-4 overflow-y-auto custom-scrollbar">
            ...
            <div className="bg-white rounded-2xl p-4">
              ...
              <div className="mt-4 pt-4 border-t border-[#E5E5EA]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-[#1D1D1F]">{user?.isPremium ? 'Pro' : 'Free'}</span>
                </div>
                {/* No upgrade/downgrade UI in MVP */}
              </div>
            </div>
            {/* Feature Toggles */}
            <div className="bg-white rounded-2xl overflow-hidden">
              <div className="px-4 py-3 bg-[#1D1D1F] flex items-center gap-2">
                ...
                <span className="text-white text-sm font-bold tracking-tight">Features</span>
              </div>
              <div className="divide-y divide-[#E5E5EA]">
                <ToggleRow
                  ...
                  checked={syncEnabled}
                  onChange={() => handleToggle(syncEnabled, setSyncEnabled)}
                  requiresPro={false}
                />
                <ToggleRow
                  ...
                  onChange={() => handleToggle(reportsEnabled, setReportsEnabled)}
                  requiresPro={false}
                />
                <ToggleRow
                  ...
                  checked={edgeSwipeEnabled}
                  onChange={() => handleToggle(edgeSwipeEnabled, setEdgeSwipeEnabled)}
                  requiresPro={false}
                />
              </div>
            </div>
